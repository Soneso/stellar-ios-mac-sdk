...
final class SorobanRpcErrorTest: XCTestCase {
    ...
    func testSorobanRpcErrorSendableConformance() {
        ...
        // This test primarily validates that the struct compiles with Sendable conformance
        // We can verify by passing it to an async context
        Task {
            let _ = error.code
            let _ = error.message
            let _ = error.data
        ...
        XCTAssertTrue(true) // If compilation succeeds, Sendable conformance is valid
    }
    ...
    func testSorobanRpcErrorIntegration() async {
        ...
        case .failure(let error):
            ...
            case .errorResponse(let rpcError):
                ...
                print("Received RPC error: code=\(rpcError.code), message=\(rpcError.message ?? "none"), data=\(rpcError.data ?? "none")")
        ...
        case .failure(let error):
            ...
            case .errorResponse(let rpcError):
                XCTAssertNotEqual(rpcError.code, 0, "Error should have a non-zero code")
                print("Received RPC error for getLedgerEntries: code=\(rpcError.code), message=\(rpcError.message ?? "none")")
            case .requestFailed(let message):
                print("Request failed for getLedgerEntries: \(message)")
            case .parsingResponseFailed(let message, _):
                XCTFail("Should not fail to parse error response: \(message)")
            }
        }
