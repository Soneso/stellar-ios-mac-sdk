...
class TransferServerTestCase: XCTestCase {
    ...
    func testDepositBankSuccess() {
        let request = DepositRequest(assetCode: "USD", account: successBankDepositAccount)
        let expectation = XCTestExpectation(description: "Test deposit bank succcess")
    ...
    func testDepositRippleSuccess() {
        let request = DepositRequest(assetCode: "XRP", account: successRippleDepositAccount)
        let expectation = XCTestExpectation(description: "Test deposit ripple succcess")
    ...
    func testDepositMXNSuccess() {
        var request = DepositRequest(assetCode: "MXN", account: successMXNDepositAccount)
        request.amount = "120"
        
        let expectation = XCTestExpectation(description: "Test deposit MXN succcess")
    ...
    func testDepositExchangeBankSuccess() {
        ...
        transferServerService.depositExchange(request: request) { (response) -> (Void) in
            switch response {
            case .success(let response):
                XCTAssertEqual("9421871e-0623-4356-b7b5-5996da122f3e", response.id)
                XCTAssertEqual("Make a payment to Bank: 121122676 Account: 13719713158835300", response.how)
    ...
    func testWithdrawExchangeSuccess() {
        ...
        transferServerService.withdrawExchange(request: request) { (response) -> (Void) in
            switch response {
            case .success(let info):
                XCTAssertEqual("GCIBUCGPOHWMMMFPFTDWBSVHQRT4DIBJ7AD6BZJYDITBK2LCVBYW7HUQ", info.accountId)
                XCTAssertEqual("id", info.memoType)
    ...
    func testSingleTxSuccess() {
        ...
        let expectation = XCTestExpectation(description: "Test single tx succcess")
        
        transferServerService.getTransaction(request: request) { (response) -> (Void) in
            switch response {
            case .success(let response):
    ...
    func testFeeSuccess() {
        ...
        let expectation = XCTestExpectation(description: "Test fee succcess")
        
        transferServerService.fee(request: request) { (response) -> (Void) in
            switch response {
            case .success(let info):
