...
public final class MethodOptions: Sendable {
    ...
    /// Selects the credential arm the simulation records. Default true.
    ///
    /// When `true` (the default), the simulation requests `ADDRESS_V2` credential arms; when
    /// `false`, legacy `ADDRESS` arms. The flag is forwarded to the simulation request and
    /// always sent in the JSON-RPC params. Set to `false` on a network below protocol 27,
    /// where V2 arms invalidate the transaction. RPC servers without protocol-27 support
    /// silently ignore this flag and return legacy `ADDRESS` entries.
    ...
    ///   - useUpgradedAuth: Request protocol-27 V2 credential arms in the simulation response; false requests legacy ADDRESS arms. Default true.
    ///
    public init(fee: UInt32 = Transaction.minBaseFee, timeoutInSeconds: UInt64 = NetworkConstants.DEFAULT_TIMEOUT_SECONDS, simulate: Bool = true, restore: Bool = false, useUpgradedAuth: Bool = true) {
        self.fee = fee
