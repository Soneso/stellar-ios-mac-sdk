    ...
    func pendingAndDuplicate() async {
        ...
        case .success(let accountResponse):
            ...
            case .success(let submitAsyncResponse):
                // Accept PENDING or DUPLICATE - transaction may already exist from previous test run
                XCTAssertTrue(submitAsyncResponse.txStatus == "PENDING" || submitAsyncResponse.txStatus == "DUPLICATE",
                              "Expected PENDING or DUPLICATE, got \(submitAsyncResponse.txStatus)")

                // Only test duplicate submission if first was PENDING
                if submitAsyncResponse.txStatus == "PENDING" {
                    let response = await sdk.transactions.submitAsyncTransaction(transaction: transaction)
                    switch response {
                    case .success(let submitAsyncResponse2):
                        // Accept PENDING or DUPLICATE - timing can vary
                        XCTAssertTrue(submitAsyncResponse2.txStatus == "PENDING" || submitAsyncResponse2.txStatus == "DUPLICATE",
                                      "Expected PENDING or DUPLICATE, got \(submitAsyncResponse2.txStatus)")
