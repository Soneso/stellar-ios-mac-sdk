...
class AccountLocalTestCase: XCTestCase {
    ...
    func testAccountNotFound() async {
        let accDetailsResEnum = await sdk.accounts.getAccountDetails(accountId: "AAAAA");
    ...
    func testGetAccountDetails() async {
        ...
        case .failure(let error):
            StellarSDKLog.printHorizonRequestErrorMessage(tag:"testGetAccountDetails()", horizonRequestError: error)
            XCTFail()
        }
    }
