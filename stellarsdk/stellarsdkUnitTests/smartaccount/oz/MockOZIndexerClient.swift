...
/// Recording test double for ``OZIndexerClient`` used by kit lifecycle
/// tests.
///
/// Subclasses the real client so the kit's `internal init(...)` accepts the
/// mock wherever it expects an ``OZIndexerClient?`` property. Overrides
...
/// methods — every call to a network endpoint falls through to the real
/// client and fails as it would in production, because the kit close-flow
/// tests never reach the network surface.
final class MockOZIndexerClient: OZIndexerClient, @unchecked Sendable {
    ...
    /// Creates a recording mock with a fake-but-valid indexer URL.
    ///
    ...
    /// recording avoids triggering an HTTP method.
