...
- [WebAuthn Setup](./smart_accounts_webauthn.md) — `WebAuthnProvider` implementations on iOS / macOS, `OZStorageAdapter` (Keychain / UserDefaults), allow-credential handling.
...
`OZSmartAccountConfig` is a struct with four required fields and several optional ones. Its initializer is `throws`: it validates inputs and throws `SmartAccountConfigurationException` on invalid values.
...
//          The initializer throws SmartAccountConfigurationException.InvalidConfig otherwise.
...
| `storage` | `OZStorageAdapter` | `OZInMemoryStorageAdapter()` | Credential/session persistence. The default is tests-only — see warning below |
...
```swift
public struct OZCreateWalletResult {
...
//        Throws SmartAccountValidationException.InvalidInput before any WebAuthn or network side effect.
// CORRECT:
let wallet = try await kit.walletOperations.createWallet(
    userName: "Alice",
    autoSubmit: true,
...
public struct OZDeployPendingResult {
...
`createWallet` and `deployPendingCredential` throw from `WebAuthnException`, `SmartAccountValidationException`, `SmartAccountTransactionException`, `SmartAccountCredentialException`, or `SmartAccountStorageException`. See [Error Handling](#error-handling).
...
`OZConnectWalletOptions` is a top-level public struct.

```swift
public struct OZConnectWalletOptions {
...
| `credentialId` [+ `contractId`] | Direct connect, skip session and WebAuthn | Non-`nil` on success; throws `SmartAccountWalletException.NotFound` if the contract does not exist on-chain |
...
```swift
public enum OZConnectWalletResult {
...
1. **Local storage.** A storage hit means the deployment is `pending` or `failed`. A `failed` entry throws `SmartAccountWalletException.NotFound` with a message pointing to `deployPendingCredential()` for retry. A `pending` entry's stored `contractId` is used directly.
...
3. **Indexer fallback** (when configured). Looks up contracts where the passkey is registered as a signer:
   - 0 contracts → throws `SmartAccountWalletException.NotFound`.
...
public struct OZAuthenticatePasskeyResult {
    public let credentialId: String          // Base64URL, no padding
...
public struct OZDelegatedSigner: OZSmartAccountSigner, Equatable, Hashable {
    ...
    public init(address: String) throws   // throws SmartAccountValidationException.InvalidAddress on bad strkey
}
```
...
```swift
public struct OZTransactionResult {
...
Throws `SmartAccountWalletException.NotConnected` when no wallet is connected, `SmartAccountValidationException` for a bad recipient or amount, `SmartAccountTransactionException.*` for simulation/submission failures, and `WebAuthnException.*` on a cancelled biometric prompt.
...
`OZResolveContextRuleIds` is `@Sendable (SorobanAuthorizationEntryXDR, Int) async throws -> [UInt32]`. It overrides automatic context-rule resolution for a single auth entry — see [smart_accounts_policies.md](./smart_accounts_policies.md).
...
Use this after `createWallet(autoSubmit: true, autoFund: false)` to defer funding, or to top up an existing wallet during development. `createWallet(autoFund: true, nativeTokenContract:)` calls it internally. Throws `SmartAccountWalletException.NotConnected`, `SmartAccountValidationException.InvalidAddress`, or `SmartAccountTransactionException.*`.
...
```swift
public enum OZSubmissionMethod {
    case relayer
    case rpc
}
...
// Forcing .relayer with no relayer configured throws SmartAccountTransactionException.SubmissionFailed.
```
...
```swift
public struct OZStoredCredential {
    public let credentialId: String                       // Base64URL, no padding
    public let publicKey: Data                            // 65 bytes (uncompressed secp256r1)
    public let contractId: String?
    public let deploymentStatus: OZCredentialDeploymentStatus // default .pending
...
public enum OZCredentialDeploymentStatus: String {
...
`deleteCredential` first runs `sync` to confirm the contract is not on-chain. When the wallet already exists on-chain it throws `SmartAccountCredentialException.Invalid` because the local entry is no longer authoritative.
...
`config.storage` defaults to `OZInMemoryStorageAdapter` (non-persistent, tests-only). Production apps supply an `OZStorageAdapter`:
...
`OZKeychainStorageAdapter` (encrypted, Apple Keychain) and `OZUserDefaultsStorageAdapter` (non-sensitive metadata) ship with the SDK. See [smart_accounts_webauthn.md](./smart_accounts_webauthn.md) for adapter selection.
...
|---|---|---|
| Wallet / `G…` | `kit.externalSigners.addFromSecret(secretKey:)` at runtime | `config.externalWallet` (`OZExternalWalletAdapter`) at kit construction |
...
```swift
public struct OZExternalSignerInfo {
...
public enum OZExternalSignerType: String {
...
public protocol OZExternalWalletAdapter: AnyObject, Sendable {
    func connect() async throws -> OZConnectedWallet?            // nil if the user cancelled
    func disconnect() async throws
    func disconnectByAddress(address: String) async throws     // default: no-op
    func signAuthEntry(
        preimageXdr: String,                                   // Base64 HashIDPreimage XDR
        options: OZSignAuthEntryOptions?                         // options.address selects the signer
    ) async throws -> OZSignAuthEntryResult                       // .signedAuthEntry = Base64 raw 64-byte Ed25519 sig
...
`addEd25519FromRawKey` throws `SmartAccountValidationException.InvalidInput` when `secretKeyBytes` is not exactly 32 bytes or `verifierAddress` is not a valid `C…` strkey. For hardware wallets, HSMs, or remote signers, supply `config.externalEd25519Adapter` so the raw seed never enters process memory:
...
`kit.events` is an `OZSmartAccountEventEmitter`. Subscribe to a specific event type via `on(_:listener:)`, to all events via `addListener(_:)`, or once via `once(_:listener:)`.
...
```swift
let unsub: OZSmartAccountEventUnsubscribe = kit.events.on(.walletConnected) { event in
...
`success == true` on `transactionSubmitted` means the network accepted the transaction for inclusion — not that it was confirmed in a ledger. Use `OZTransactionResult.success` (from `transfer`/`contractCall`) for confirmed state.
...
The initializer throws `SmartAccountConfigurationException.InvalidConfig` for a blank, non-HTTPS (except `http://localhost`), or host-less URL. `close()` releases the underlying `URLSession`; the kit handles this automatically when the client is reached via `kit.indexerClient`.
...
Throws `SmartAccountValidationException.InvalidInput` when `credentialId` is not valid Base64URL, `SmartAccountIndexerException.RequestFailed` on HTTP errors, or `SmartAccountIndexerException.Timeout`.
...
- Audit the `storage` adapter — `OZInMemoryStorageAdapter` silently loses credentials on process exit, permanently locking users out of mainnet funds.
...
|------|-------|-----------------|
| `SmartAccountConfigurationException` | 1001–1002 | `InvalidConfig`, `MissingConfig` |
| `SmartAccountWalletException` | 2001–2003 | `NotConnected`, `AlreadyExists`, `NotFound` |
| `SmartAccountCredentialException` | 3001–3004 | `NotFound`, `AlreadyExists`, `Invalid`, `DeploymentFailed` |
| `WebAuthnException` | 4001–4004 | `RegistrationFailed`, `AuthenticationFailed`, `NotSupported`, `Cancelled` |
| `SmartAccountTransactionException` | 5001–5004 | `SimulationFailed`, `SigningFailed`, `SubmissionFailed`, `Timeout` |
| `SmartAccountSignerException` | 6001–6002 | `NotFound`, `Invalid` |
| `SmartAccountValidationException` | 7001–7003 | `InvalidAddress`, `InvalidAmount`, `InvalidInput` |
| `SmartAccountStorageException` | 8001–8002 | `ReadFailed`, `WriteFailed` |
| `SmartAccountSessionException` | 9001–9002 | `Expired`, `Invalid` |
| `SmartAccountIndexerException` | 10001–10002 | `RequestFailed`, `Timeout` |
...
} catch let e as SmartAccountConfigurationException.MissingConfig {
...
} catch let e as SmartAccountWalletException.NotFound {
