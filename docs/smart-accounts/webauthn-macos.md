...
The same `rpId` MUST be passed to both the provider and the config: the provider value is what the system passkey UI uses, the config value is read by the kit when relevant for auth-entry encoding and indexing.
...
- **`KeychainStorageAdapter`**: production storage backed by macOS Keychain Services. Custom `serviceName` for scoping.
...
Sandboxed apps may need `com.apple.security.keychain-access-groups` for Keychain access. Use `UserDefaultsStorageAdapter` if this is constrained.
