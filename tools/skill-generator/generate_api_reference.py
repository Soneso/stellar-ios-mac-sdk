...
def extract_type_members(body: str, type_name: str,
    ...
    # A deprecation attribute may arrive as its own chunk (the splitter treats an
    # attribute line as a declaration start), so a found marker is carried
    # forward onto the next real declaration.
    pending_dep_note = ""

    for decl in declarations:
        decl = compact_whitespace(decl)

        # Capture a deprecation marker before attributes are stripped, so the
        # listing carries the replacement the SDK names. The attribute's
        # argument list is removed with paren balancing: a `message:` string
        # may itself contain parentheses, which RE_ATTRIBUTE cannot span.
        dep_note = pending_dep_note
        mavail = re.search(r'@available\s*\(', decl)
        if mavail:
            close = balance(decl, mavail.end() - 1, '(', ')')
            attr_text = decl[mavail.start():close + 1]
            if 'deprecated' in attr_text:
                ren = re.search(r'renamed:\s*"([^"]+)"', attr_text)
                dep_note = (f"  // deprecated, use {ren.group(1)}" if ren
                            else "  // deprecated")
            decl = decl[:mavail.start()] + decl[close + 1:]

        # Remove attributes
        decl = RE_ATTRIBUTE.sub('', decl).strip()
        if not decl:
            # Pure-attribute chunk: keep the marker for the declaration it precedes.
            pending_dep_note = dep_note
            continue
        pending_dep_note = ""
        ...
        if ctx_kind == "enum":
            ...
            if mc:
                ...
                for entry in split_case_entries(raw):
                    info.cases.append(f"case {entry}{dep_note}")
                continue
        ...
        if m:
            info.typealiases.append(f"typealias {m.group(1)} = {m.group(2)}{dep_note}")
            continue
        ...
        if m:
            ...
            if type_hint:
                ...
                sig += f" = {rhs}"
            info.constants.append(sig + dep_note)
            continue

        # --- Properties (let/var) ---
        m = RE_PROPERTY.match(decl_no_access)
        if m:
            info.properties.append(_render_property(decl, m) + dep_note)
            continue
        ...
        if m:
            ...
            if mods_after:
                sig += " " + " ".join(mods_after)
            info.initializers.append(sig + dep_note)
            continue
        ...
        if m:
            ...
            if return_type:
                sig += f" -> {return_type}"
            info.methods.append(sig + dep_note)
            continue
        ...
        if m:
            ...
            if return_type:
                sig += f" -> {return_type}"
            info.subscripts.append(sig + dep_note)
            continue
...
def main() -> None:
    ...
    # Group-level table of contents. Anchors follow GitHub's slug rules
    # (lowercase, spaces to hyphens, punctuation dropped), so each entry
    # resolves to the "## <title>" heading emitted below.
